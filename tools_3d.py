...
def reconstruct(tilt_data,tilt_angles,algorithm='ASTRA_SIRT',iterations=20):
    ...
    if algorithm == 'ASTRA_SIRT':
        recon = astrarecon(tilt_data,tilt_angles,iterations)
...
def astrarecon(tilt_data,tilt_angles,iterations=1):
    ...
    proj_geom = astra.create_proj_geom('parallel3d', 1.0, 1.0, proj_shape[1], proj_shape[2], angles)
...
def project(reconstruction,angles):
    ...
    for angle in angles:
        ...
        for x in xrange(np.shape(rot_recon)[0]):
            for y in xrange(np.shape(rot_recon)[1]):
...
def bin3d(data_in,x_bin=2,y_bin=2,z_bin=2,binning='Average'):
    ...
    for x in xrange(dshape[0]/x_bin):
        for y in xrange(dshape[1]/y_bin):
            for z in xrange(dshape[2]/z_bin):
                ...
                for x_i in xrange(x_bin):
                    for y_i in xrange(y_bin):
                        for z_i in xrange(z_bin):
...
def generate_sinogram(tilt_series):
    ...
    for n in xrange(len(tilt_series)):
